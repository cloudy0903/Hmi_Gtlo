...
if (isset($_GET['hapus']) && isset($_GET['id'])) {
    ...
    $query = "DELETE FROM pengurus_komisariat WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>
                Swal.fire({
                    ...
                    window.location.href = 'kelola_pengurus_komisariat.php';  // Redirect kembali ke daftar
                });
              </script>";
    } else {
...
<body>
    ...
    <div class="project_area">
        <p class="tulisan_input2">DAFTAR PENGURUS KOMISARIAT HMI</p>

        <div class="d-flex justify-content-between mb-3">
            <div>
                <a class="btn btn-dark" href="cetak_pengurus_komisariat.php" target="_blank"><i class="fa fa-print"></i> Cetak</a>
                <a class="btn btn-dark" href="tambah_pengurus_komisariat.php"><i class="fa fa-user-plus"></i> Tambah</a>
            </div>
...
$query = $cari ? mysqli_query($conn, "SELECT * FROM pengurus_komisariat WHERE nama LIKE '%$cari%' OR email LIKE '%$cari%'")
    : mysqli_query($conn, "SELECT * FROM pengurus_komisariat LIMIT $posisi, $batas");
$no = 1 + $posisi;
?>
            ...
            if (mysqli_num_rows($query) > 0) {
                while ($row = mysqli_fetch_assoc($query)) {
                    ...
                    echo "<tr>
                        ...
                        <td>
                            <a class='btn btn-edit' href='edit_pengurus_komisariat.php?id={$row['id']}'><i class='fas fa-pencil-alt'></i> Edit</a>
                            <a href='kelola_pengurus_komisariat.php' class='btn btn-hapus' onclick='return hapusPengguna(event, {$row['id']})'><i class='fa fa-trash'></i> Hapus</a>
                        </td>
                    </tr>";
                }
            } else {
...
<?php
$totalData = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pengurus_komisariat"));
    ...
    function hapusPengguna(event, id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = 'kelola_pengurus_komisariat.php?hapus=true&id=' + id; // Arahkan ke halaman penghapusan
            }
        });
    }
</script>
